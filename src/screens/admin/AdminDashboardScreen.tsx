...
import { Bell, Shield, UserPlus, ClipboardList } from 'lucide-react-native';
...
import { ADMIN_USER, REGISTRATIONS, AUDIT } from '../../data/mock';
...
export function AdminDashboardScreen({ onTabChange, onBell }: Props) {
  return (
        ...
        <SafeAreaView edges={['top']} style={styles.hero}>
          ...
          <View style={styles.heroTop}>
            <View style={styles.greetRow}>
              <Avatar size={36} name={ADMIN_USER.name} variant="lime" />
              <View>
                <Text style={styles.hello}>Signed in as</Text>
                <Text style={styles.greetName}>{ADMIN_USER.name}</Text>
              </View>
            </View>
            <View style={styles.heroIcons}>
              <Eyebrow dark icon={<Shield size={11} color={colors.white} />}>{ADMIN_USER.role}</Eyebrow>
              <IconBtn dark dot onPress={onBell}><Bell size={18} color={colors.white} /></IconBtn>
            </View>
          </View>

          <Text style={styles.h1}>
            14 items <Text style={{ color: colors.accent }}>need you</Text>
          </Text>
          <Text style={styles.heroSub}>Approvals are waiting. Older queue items are highlighted.</Text>
          ...
          <View style={styles.cardsRow}>
            <Pressable style={styles.queueCard} onPress={() => onTabChange('registrations')}>
              ...
              <Text style={styles.queueVal}>6</Text>
            ...
            <Pressable style={styles.queueCard} onPress={() => onTabChange('enrolments')}>
              ...
              <Text style={styles.queueVal}>4</Text>
        ...
        <View style={styles.body}>
          ...
          <View style={styles.list}>
            {REGISTRATIONS.filter(r => r.status === 'pending').slice(0, 4).map((r, i, arr) => (
              <View key={r.id} style={[styles.listItem, i < arr.length - 1 && styles.listItemBorder]}>
          ...
          <View style={styles.list}>
            {AUDIT.slice(0, 3).map((l, i, arr) => {
              ...
              return (
                <View key={l.id} style={[styles.listItem, i < arr.length - 1 && styles.listItemBorder]}>
                  <View style={[styles.activityIcon, { backgroundColor: colors.lightGray }]}>
                    <Icon size={14} color={toneFg(l.tone)} />
                  </View>
                  <View style={{ flex: 1 }}>
